...
const DEFAULT_EMOTION_SETTINGS = {
  Happy: { fontSize: 65, position: "하단", effect: "바운스", font: "통통체" },
  Angry: { fontSize: 70, position: "하단", effect: "쉐이크", font: "각진체" },
  Sad:   { fontSize: 45, position: "하단", effect: "페이드", font: "얇은체" },
...
const getCaptionPositionStyle = (position) => {
  if (position === "상단") return { top: "8%", bottom: "auto", transform: "translateX(-50%)" };
  if (position === "중앙") return { top: "50%", bottom: "auto", transform: "translate(-50%, -50%)" };
  return { bottom: "8%", top: "auto", transform: "translateX(-50%)" };
};
...
export default function EditPage() {
  ...
  const updateCurrentEmotionSetting = (key, value) => {
    setEmotionSettings((prev) => ({
      ...prev,
      [selectedEmotion]: { ...prev[selectedEmotion], [key]: value },
    }));
  };
  ...
  const selectEmotion = (emotion) => {
    ...
    setSegments((prev) =>
      prev.map((seg, i) => (i === selectedIndex ? { ...seg, emotion } : seg))
    );
  };
  ...
  const scrollScript = (direction) => {
    scriptListRef.current?.scrollBy({ top: direction * 80, behavior: "smooth" });
  };
      ...
      <div className="edit-body">
        <aside className="edit-left">
          <div className="panel-header-row">
            ...
            <button type="button" className="panel-close-btn">✕</button>
          </div>
          ...
          <div className="style-settings-box">
            <div className="setting-group">
              <div className="setting-label-row">
                ...
                <span className="font-size-value">{currentSetting.fontSize}px</span>
              </div>
              <input
                type="range" min={10} max={100}
                value={currentSetting.fontSize}
                onChange={(e) => updateCurrentEmotionSetting("fontSize", Number(e.target.value))}
              ...
              <div className="pill-group">
                {["하단", "중앙", "상단"].map((item) => (
                  <button
                    ...
                    className={`pill-btn${currentSetting.position === item ? " selected" : ""}`}
                    onClick={() => updateCurrentEmotionSetting("position", item)}
                  >{item}</button>
                ))}
              </div>
            </div>
            ...
            <div className="setting-group">
              ...
              <div className="pill-group">
                {["바운스", "페이드", "쉐이크", "없음"].map((item) => (
                  <button
                    type="button" key={item}
                    className={`pill-btn${currentSetting.effect === item ? " selected" : ""}`}
            ...
            <div className="setting-group">
              ...
              <div className="pill-group">
                {FONT_OPTIONS.map((item) => (
                  <button
                    type="button" key={item}
                    className={`pill-btn${currentSetting.font === item ? " selected" : ""}`}
        ...
        <main className="edit-center">
          <div className="video-area">
            {videoSrc ? (
              ...
              <div className="video-frame empty-video">영상을 불러오는 중입니다</div>
            )}

            {captionText && (
              <div
                className={`caption-preview ${EFFECT_CLASS[captionSetting.effect]}`}
                style={{
                  color: EMOTION_COLORS[captionEmotion] ?? "#FFFFFF",
                  fontSize: `${captionSetting.fontSize}px`,
                  fontFamily: "Arial, sans-serif",
                  fontWeight: captionSetting.font === "고딕" ? 700 : 900,
        ...
        <aside className="edit-right">
          ...
          <button type="button" className="scroll-arrow" onClick={() => scrollScript(-1)}>▲</button>

          <div className="script-list" ref={scriptListRef}>
            {segments.length === 0 ? (
              <div className="script-empty-message">분석된 자막이 없습니다.</div>
            ) : (
              segments.map((segment, index) => (
                <button
                  type="button" key={index}
                  className={`script-item${selectedIndex === index ? " selected-script" : ""}`}
                ...
                >
                  ...
                  <div className="script-info">
                    ...
                    <span className="script-emotion"
                      style={{ color: EMOTION_COLORS[segment.emotion] ?? "#FFFFFF" }}>
          ...
          <div className="script-footer">
            <button type="button" className="done-btn" onClick={handleCompleteEdit}>
              편집 완료
            </button>
          </div>
        </aside>
